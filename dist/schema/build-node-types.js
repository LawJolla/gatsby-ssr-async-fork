...
module.exports = async () => {
  ...
  function createNodeFields(type) {
    ...
    };const childNodesByType = _(type.nodes).flatMap(({ children }) => children.map(getNode)).groupBy(node => node.internal ? _.camelCase(node.internal.type) : undefined).value();
    ...
    Object.keys(childNodesByType).forEach(childNodeType => {
      ...
      if (maxChildCount > 1) {
        defaultNodeFields[_.camelCase(`children ${childNodeType}`)] = {
          ...
          resolve(node, a, { path }) {
            const filteredNodes = node.children.map(id => getNode(id)).filter(({ internal }) => _.camelCase(internal.type) === childNodeType);

            // Add dependencies for the path
            filteredNodes.forEach(n => createPageDependency({ path, nodeId: n.id }));
        ...
        defaultNodeFields[_.camelCase(`child ${childNodeType}`)] = {
          ...
          resolve(node, a, { path }) {
            const childNode = node.children.map(id => getNode(id)).find(({ internal }) => _.camelCase(internal.type) === childNodeType);
  ...
  async function createType(nodes, typeName) {
    ...
    const proccesedType = (0, _extends3.default)({}, intermediateType, {
